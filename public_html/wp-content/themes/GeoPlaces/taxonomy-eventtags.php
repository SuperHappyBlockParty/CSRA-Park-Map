...
if(get_option('ptthemes_category_map_event') == 'yes' || get_option('ptthemes_category_map_event') == ''){
	...
	$category_link = get_term_link( $current_term->slug, CUSTOM_TAG_TYPE2 );
...
if ($childCatID){
	...
	foreach ($childCatID as $kid) {
		...
		$cat_link = get_term_link($childCatName->slug, CUSTOM_TAG_TYPE2);
		?>
...
$category_main_link = get_term_link($current_term->slug, CUSTOM_TAG_TYPE2);
?>
<ul class="sort_by">
