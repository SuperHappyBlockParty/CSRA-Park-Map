...
if ($childCatID){
	...
	$i=0;
	$childCatcount = $wpdb->get_col("SELECT count FROM $wpdb->term_taxonomy WHERE parent=$deptID");
	foreach ($childCatID as $kid ) {
		$childCatName = $wpdb->get_row("SELECT name, term_id,slug FROM $wpdb->terms WHERE term_id=$kid");
		if(intval($childCatName->term_id) && $childCatcount[$i] !=0){
		$category_link = get_term_link(intval($childCatName->term_id), CUSTOM_CATEGORY_TYPE1 ); 
...
$category_main_link = get_term_link( $current_term->slug, CUSTOM_CATEGORY_TYPE1 ); 
 ?>
<ul class="sort_by">
	   ...
	   <?php } ?>
		...
		<?php } ?>
				...
				<?php echo '<p>';
					  ...
					  echo '</p>';				/* it's to show which posttype have which taxonomy */
