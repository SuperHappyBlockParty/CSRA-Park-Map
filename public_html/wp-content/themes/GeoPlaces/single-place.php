...
<div  class="<?php templ_content_css();?>" >
  ...
  <div class="entry">
    <div <?php post_class('single clear'); ?> id="post_<?php the_ID(); ?>">
     ...
     <div class="post-meta single_meta">
        ...
        <span class="post-author"><label> <?php _e('By','templatic'); ?> </label> <a href="<?php echo get_author_posts_url(get_the_author_meta('ID')); ?>" title="Posts by <?php the_author(); ?>">
        <?php the_author(); ?>
        </a> </span>
        <?php } ?>
      ...
      <div class="post-content">
		...
		the_content(); ?>
         ...
         <?php }?>
        <?php
		$special_offer = get_post_meta($post->ID,'proprty_feature',true);
		if($special_offer != ""){
		?>
        <div class="register_info">     
            <h3><?php echo SPECIAL_OFFER; ?></h3><p><?php echo get_post_meta($post->ID,'proprty_feature',true);?></p>
         </div>
        <?php } ?>
  ...
  <?php 
  //if (function_exists('dynamic_sidebar')){ dynamic_sidebar('place_detail_content_banner'); }
  $pdata = $post;
  global $pdata;
  if (function_exists('dynamic_sidebar')){ dynamic_sidebar('place_detail_content_banner'); }
    $city_id = get_post_meta($post->ID,'post_city_id',true); 
  if(get_option('ptthemes_related_listing') == 'Yes') {
	//get_related_posts($post,CUSTOM_POST_TYPE1,CUSTOM_TAG_TYPE1,CUSTOM_CATEGORY_TYPE1,$city_id); 
	get_related_posts($post,CUSTOM_POST_TYPE1,CUSTOM_TAG_TYPE1,CUSTOM_CATEGORY_TYPE1,$city_id); 
  } 
  $post = $pdata;
  comments_template();
