...
}else
{
	...
	{
		if(is_valid_coupon($_SESSION['place_info']['proprty_add_coupon']))
		{
			$payable_amount = get_payable_amount_with_coupon($payable_amount,$_SESSION['place_info']['proprty_add_coupon']);
			$is_recurring = get_property_price_info($payable_amount,$_SESSION['place_info']['is_recurring']);
		}else
		{
...
<input type="hidden" name="price_select" value="<?php echo $_REQUEST['price_select'];?>" />
	...
	}else
	{
		...
		{
			printf(GOING_TO_UPDATE_MSG, $type_title, $alive_days, display_amount_with_currency($payable_amount));
		}
		echo '</h5>';
	}
	?> 
