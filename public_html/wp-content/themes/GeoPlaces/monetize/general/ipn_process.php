...
else
{
	...
	if (eregi('VERIFIED', $info)) 
	{
		...
		$subject = get_option('post_submited_success_email_user_subject');
		if(!$subject)
		{
			$subject = __("Place Listing Submitted and Payment Success Confirmation Email","templatic");
		}
		$content = get_option('post_submited_success_email_user_content');
		if($content==''){
			$content = "<p>Dear [#to_name#],</p><p>You Submitted below information. </p><p>This email is just for your knowledge.</p><p>[#information_details#]</p><br><p>We hope you enjoy. Thanks!</p><p>[#site_name#]</p>";
		}
		$store_name = get_option('blogname');
		$search_array = array('[#to_name#]','[#information_details#]','[#site_name#]');
